...
- **Fonts**: `@fontsource/instrument-serif` (`--serif`), `@fontsource/jetbrains-mono` (`--mono`), `@fontsource/inter` (`--sans`), `@fontsource/jersey-10` (`--title-display`, pixel/arcade hero titles — Landing cards + character-page HeroCard), `@fontsource/gugi` (`--title-rounded`, case-study headings). Use the token, not the family name.
...
src/
  ...
  styles/
    ...
    reset.css
  data/
    substack.json          # generated by scripts/fetch-substack.mjs at build time (via `prebuild`)
    substack.d.ts          # types for the generated JSON
    instagram-posts.json   # curated Instagram posts shown in Explorer RightCard
  lib/
...
design-reference/          # original Codex Design handoff bundle — DO NOT EDIT, READ-ONLY
...
- **Mobile and desktop styling must stay in sync**: when changing fonts, colors, spacing, or typographic properties on desktop Landing elements (`.heroTitle`, `.cardName`, etc.), apply the equivalent change to their mobile counterparts (`.mHeroTitle`, `.mCardName`, etc.) in the same commit. The mobile layout is a separate component tree, not a media-query reflow, so changes don't propagate automatically.
...
npm run build        # runs `prebuild` (scripts/fetch-substack.mjs) → tsc -b && vite build
